...
def piano_roll_to_midi(piece):
  ...
  for voice in tracks:
    ...
    tracks[voice].append(mido.Message(
      'program_change', program=1, time=0))
...
def save_heatmaps(model, y, C, chord_id, step, device):
  """
  ...
  chord_id = torch.tensor(chord_id).unsqueeze(0).to(device)
  model.eval()
  with torch.no_grad():
    out = model.forward(x, C2, chord_id).view(I, T, P).cpu().numpy().transpose(2, 0, 1)
...
def harmonize_melody_and_save_midi(melody, id_number, chord2id, model, device):
  """
  ...
  prediction = harmonize(y, D, model, chord_id, device) + 30  # 30 back on before passing to piano_roll_to_midi
...
#   render_midi_save_wav(f'results/generated_midi/harmonize_val_set_step_{str(pad_number(id_number))}.wav')


# harmonize a melody
def harmonize(y, C, model, chord_id, device):
  """
  ...
  with torch.no_grad():
    ...
    for i in range(num_steps):
      ...
      if len(C2.shape) < 3:
        C2 = C2[None]
      if len(chord_id.shape) < 3:
        chord_id = chord_id[None]
      x = model.pred(x, C2, chord_id, device)
      if len(C2.shape) == 3:
        C2 = C2[0, :, :]
      x[C2 == 1] = x_cache[C2 == 1]
      C2 = C.copy()
...
def generate_random_chorale(model, chord2id, device):
  """
  ...
  return harmonize(y, C, model, chord_id, device)

def generate_random_chorale_and_save(model, device, chord2id=None, name='generated', render_midi=True):
  """
  Calls harmonize with random initialization and C=0, and so generates a new sample that sounds like Bach.
  """

  prediction = generate_random_chorale(model, chord2id, device) + MIN_MIDI_PITCH
  prediction = prediction.transpose().tolist()
  prediction = np.array(prediction)
  midi_output = piano_roll_to_midi(prediction)
...
class Net(nn.Module):
  ...
  def __init__(self, n_layers, hidden_size, chord_num, chord_dim, chord_num_pertrack):
    super(Net, self).__init__()
    self.initial_conv = nn.Conv2d(2 * I + 1, hidden_size, 3, padding=1)
    self.chord_emb = nn.Embedding(chord_num, chord_dim)
    self.chord_sum_linear = nn.Linear(chord_dim * chord_num_pertrack, P)
  ...
  def forward(self, x, C, chord):
    ...
    N = x.shape[0]
    T = x.shape[2]
    batch_chord_emb = torch.flatten(self.chord_emb(chord), start_dim=1, end_dim=-1)
    batch_chord_emb = self.chord_sum_linear(batch_chord_emb).unsqueeze(1).unsqueeze(1).expand(-1, 1, T, -1)
    ...
    y = torch.cat((tiled_C * x, tiled_C, batch_chord_emb), dim=1)
  ...
  def pred(self, y, C, chord_id, device, temperature=1.0, seed=100):
    x, C2 = self.expand(y, C, device)
    # plug x and C2 into the model
    rs = np.random.RandomState(seed)
    with torch.no_grad():
      out = self.forward(x, C2, chord_id).view(I, T, P).cpu().numpy()
...
class RNNNet(nn.Module):
  ...
  def __init__(self, n_layers, hidden_size, chord_num, chord_dim, chord_num_pertrack):
    ...
    self.lstm = nn.ModuleList([nn.LSTM(P, P, 2, batch_first=True) for _ in range(2 * I)])
    self.initial_conv = nn.Conv2d(2 * I + 1, hidden_size, kernel_size=1, stride=1, padding=0)
    self.h0 = torch.zeros([2, 1, P])
    self.c0 = torch.zeros([2, 1, P])
    # self.initial_conv = nn.Conv2d(2 * I + 1, hidden_size, 3, padding=1)
    self.chord_emb = nn.Embedding(chord_num, chord_dim)
    self.chord_sum_linear = nn.Linear(chord_dim * chord_num_pertrack, P)
    self.initial_batchnorm = nn.BatchNorm2d(hidden_size)
  ...
  def forward(self, x, C, chord):
    ...
    # mask x and combine it with the mask to produce a tensor of shape (N, 2*I, T, P)
    y = torch.cat((tiled_C * x, tiled_C, batch_chord_emb), dim=1)
    y = y.permute(1, 0, 2, 3).split(1, 0)
    new_y = []
    for part in y:
      output_y = self.lstm[0](part.squeeze(0))
      new_y.append(output_y[0])
    y = torch.stack(new_y).permute(1, 0, 2, 3)
